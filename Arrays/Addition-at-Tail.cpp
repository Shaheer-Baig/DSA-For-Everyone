...
int main() {
    ...
    // New element to be added at the tail
    char newElement = 'Z';

    // Check if the array has space to add a new element
    if (size < 10) {
        // Add the new element at the tail (index 'size')
    ...
    // Display the array after adding the new element at the tail
    cout << "Array after adding new element at tail: ";
    for (int i = 0; i < size; i++) {
        cout << array[i] << " ";
    }
    cout << endl;
